...
def process_with_llm(message):
    ...
    llm_url = os.environ.get('LLM_API_URL', 'http://localhost:8000/process')  # Replace with ngrok URL
    try:
...
def transcribe_recording(recording_url):
    ...
    whisper_url = os.environ.get('WHISPER_API_URL', 'http://localhost:8000/transcribe')
    try:
...
def text_to_speech(text):
    ...
    tts_url = os.environ.get('TTS_API_URL', 'http://localhost:8000/tts')
    try:
